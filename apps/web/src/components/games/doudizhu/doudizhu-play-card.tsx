...
function cardBaseClass(red: boolean, size: "mini" | "compact" | "full"): string {
  return cn(
    ...
    size === "mini"
      ? "h-16 w-12 px-2 py-1.5"
      : size === "compact"
        ? "h-24 w-[72px] px-3 py-2"
        : "h-28 w-20 px-3 py-2",
...
export function DoudizhuDisplayCard({
  ...
  return (
    <div className={cardBaseClass(red, size)}>
      ...
      <div
        className={cn(
          size === "mini"
            ? "absolute bottom-1.5 right-1.5 text-xs font-semibold"
            : size === "compact"
              ? "absolute bottom-2 right-2 text-base font-semibold"
...
export function DoudizhuPlayCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        cardBaseClass(red, "full"),
        "transition-all",
        pulse ? "scale-[1.03]" : "",
        selected
          ? "-translate-y-3 rotate-[-1deg] border-amber-400 shadow-[0_26px_60px_-24px_rgba(255,122,69,0.72)] ring-2 ring-amber-300/45"
          : "border-slate-300/90 hover:-translate-y-1.5 hover:rotate-[-1deg] hover:shadow-[0_22px_55px_-24px_rgba(255,122,69,0.55)]",
