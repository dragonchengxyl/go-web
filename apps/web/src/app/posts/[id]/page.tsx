...
function TipModal({
  ...
  async function handleTip() {
    ...
    try {
      ...
      const { pay_url } = await apiClient.payTipAlipay(
        order.id,
...
function AuthorCard({
  ...
  useEffect(() => {
    ...
    if (apiClient.getToken() && currentUserId && currentUserId !== authorId) {
      apiClient
        .getFollowers(authorId, 1, 1000)
        .then((res) => {
          setIsFollowing(
            res.followers?.some((f) => f.follower_id === currentUserId) ??
              false,
          );
        })
        .catch(() => {});
    ...
    <div className="bg-card border rounded-xl p-5 mt-6">
      <div className="flex items-start gap-3">
        <Link href={`/users/${author.id}`} className="flex-shrink-0">
          <div
            className={`w-12 h-12 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-white font-bold hover:brightness-110 transition-all`}
          >
            ...
            {!isSelf && apiClient.getToken() && (
              <Button
                ...
                className={
                  isFollowing
                    ? ""
                    : "bg-gradient-to-r from-brand-purple to-brand-teal text-white border-0 hover:brightness-110 flex-shrink-0"
                }
              >
                {isFollowing ? (
                  <>
...
function SidebarMorePosts({
  ...
  useEffect(() => {
    if (!authorId) return;
    apiClient
      .getUserPosts(authorId, 1, 7)
      .then((res) => {
        setPosts(
          (res.posts ?? []).filter((p) => p.id !== currentPostId).slice(0, 6),
        );
      })
      .catch(() => {});
  ...
  return (
    <div className="mt-8">
      ...
      <div className="grid grid-cols-2 gap-3">
        {posts.map((p) => (
          <PostGalleryCard key={p.id} post={p} />
        ))}
      </div>
    </div>
  );
      ...
      {post.moderation_status === "pending" &&
        ...
        !justSubmitted && (
          <div className="mb-4 flex items-start gap-2.5 px-4 py-3 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-yellow-700 dark:text-yellow-400 text-sm">
            <span className="mt-0.5 text-base leading-none">⏳</span>
            <p>
              您的帖子正在审核中，审核通过后将对其他用户可见。通常在 24
              小时内完成。
            </p>
          </div>
        )}
      ...
      {post.moderation_status === "blocked" &&
        currentUserId === post.author_id && (
          <div className="mb-4 flex items-start gap-2.5 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-400 text-sm">
            <span className="mt-0.5 text-base leading-none">🚫</span>
            <p>
              这条帖子未通过社区审核，目前仅自己可见。你可以修改后重新发布。
            </p>
          </div>
        )}
      ...
      <div className="flex items-center gap-1 mt-3 mb-6">
        ...
        {isLoggedIn && post.author_id && (
          <Button
            ...
            className="text-yellow-600 hover:text-yellow-700 ml-auto"
          >
        ...
        {comments.map((c) => (
          <div key={c.id} className="flex gap-3">
            <Link href={`/users/${c.user_id}`}>
              <div
                className={`w-8 h-8 rounded-full bg-gradient-to-br ${hashGradient(c.user_id)} flex items-center justify-center flex-shrink-0 hover:brightness-110 transition-all`}
              >
