...
import { ArrowLeft, Calendar, MapPin, Globe, Users, CheckCircle2, Clock } from 'lucide-react';
...
export default function EventDetailPage() {
  ...
  if (!event) {
    return (
      <div className="max-w-2xl mx-auto pt-20 px-4 text-center py-16 text-muted-foreground">
        ...
        <Link href="/events"><Button variant="outline">返回活动列表</Button></Link>
      </div>
    );
  }
  ...
  const isFull = event.max_capacity > 0 && event.attendee_count >= event.max_capacity;
    ...
    <div className="max-w-2xl mx-auto pt-20 px-4 pb-12">
      ...
      <Link href="/events">
        <Button variant="ghost" size="sm" className="mb-6 -ml-2">
          <ArrowLeft className="h-4 w-4 mr-1" />返回活动
        </Button>
      </Link>
        ...
        <div className="h-32 bg-gradient-to-br from-brand-purple/40 via-brand-teal/30 to-brand-coral/20 relative">
          <div className="absolute top-3 right-3">
            <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
              isPublished ? 'bg-green-500/20 text-green-500 border border-green-500/30' :
              event.status === 'cancelled' ? 'bg-red-500/20 text-red-500 border border-red-500/30' :
              'bg-muted text-muted-foreground border border-border'
            }`}>
              {isPublished ? '报名中' : event.status === 'cancelled' ? '已取消' : event.status === 'completed' ? '已结束' : '草稿'}
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {attendees.map(a => (
              <Link key={a.user_id} href={`/users/${a.user_id}`}>
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-purple to-brand-teal flex items-center justify-center text-xs text-white font-bold hover:brightness-110 transition-all"
                  title={a.user_id}>
