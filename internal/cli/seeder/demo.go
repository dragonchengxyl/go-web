...
const (
	// DemoPassword is the shared password for seeded demo accounts.
	DemoPassword = "Passw0rd123"
	// DefaultDemoAdminEmail is the seeded admin login used by smoke tests.
	DefaultDemoAdminEmail = "frostfang@furry.local"
)
...
// DemoAccounts returns the canned demo accounts exposed by the seed.
func DemoAccounts() []DemoAccount {
	return []DemoAccount{
		{Email: "frostfang@furry.local", Role: "admin"},
		{Email: "silvertail@furry.local", Role: "creator"},
		{Email: "mosspaws@furry.local", Role: "member"},
		{Email: "pixellynx@furry.local", Role: "member"},
	}
}

// SeedDemo inserts or updates a stable set of demo data.
func SeedDemo(ctx context.Context, cfg *configs.Config, out io.Writer) error {
	...
	for _, item := range users {
		...
		if err != nil {
			return fmt.Errorf("upsert user %s: %w", item.Email, err)
		}
	...
	for _, item := range groups {
		if err := upsertGroup(ctx, tx, item, userIDs[item.OwnerEmail], now); err != nil {
			return fmt.Errorf("upsert group %s: %w", item.Name, err)
		}
	}
	...
	for _, item := range groups {
		...
		if _, err := tx.Exec(ctx, `
			...
			WHERE id = $1
		`, groupID, now); err != nil {
			return fmt.Errorf("update group member count: %w", err)
		}
	}
	...
	for _, item := range events {
		if err := upsertEvent(ctx, tx, item, userIDs[item.OrganizerEmail], now); err != nil {
			return fmt.Errorf("upsert event %s: %w", item.Title, err)
		}
	}
	...
	for _, item := range eventAttendees {
		...
		`, eventID, userIDs[item.Email], item.Status, now); err != nil {
			return fmt.Errorf("insert event attendee: %w", err)
		}
	}
	for _, item := range events {
		...
		if _, err := tx.Exec(ctx, `
			...
			WHERE id = $1
		`, eventID, now); err != nil {
			return fmt.Errorf("update event attendee count: %w", err)
		}
	}
	...
	for _, item := range posts {
		if err := upsertPost(ctx, tx, item, userIDs[item.AuthorEmail], now); err != nil {
			return fmt.Errorf("upsert post %s: %w", item.Title, err)
		}
	}
	...
	fmt.Fprintln(out, "seeded demo data successfully")
	fmt.Fprintln(out, "sample accounts:")
	for _, item := range DemoAccounts() {
		fmt.Fprintf(out, "- %s / %s (%s)\n", item.Email, DemoPassword, item.Role)
	}
	return nil
}
